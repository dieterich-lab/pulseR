...
plotDNA <- function(t,q0){
  n <- dim(DNA1)[1]
  q1 <- q0 +
    geom_line(data=DNA1[1:round(t*n),],
              aes(x=x, y=y1), color="green", alpha=.7, size=lineWidth) +
    geom_line(data=DNA1[1:round(t*n),], aes(x=x, y=y2), color="green", alpha=.8, size=lineWidth)
}

plotPulse <- function(t, q1){
  n <- dim(pulse)[1]
