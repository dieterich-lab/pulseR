...
opts <- setTolerance(params = 1e-6, normFactors = 1e-3, options = opts)
...
geneIndex <- 7
parName <- "b"
interval <- c(0.95,1.05) * fit[[parName]][geneIndex]
numPoints <- 51
pl <- profileGene(parName, geneIndex, pd, fit, interval, options=opts, numPoints = 21)
```
...
plTotal <- profile(paramPath = list("b",geneIndex),
              pd = pd, 
              par = fit,
              options = opts,
              interval = interval,
              numPoints = 20)
plotPL(pl)
lines(plTotal$b, plTotal$logL, col="tomato", lty=2, lwd=2)
```
