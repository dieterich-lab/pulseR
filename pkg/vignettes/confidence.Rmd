...
CIs <- estimateCIFixed(pd =pd, fit = fit, geneIndexes = geneIndexes, 
                       parName = parName, options =  options) 
```

```{r}
plot(x = geneIndexes, y=fit$b, ylim = c(.8*min(CIs), 1.2*max(CIs)))
arrows(geneIndexes, CIs[,1], geneIndexes, CIs[,2], angle = 90, code = 3, length = .05)
```
